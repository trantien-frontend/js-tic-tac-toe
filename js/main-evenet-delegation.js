...
function InitCellElementList() {
  ...
  const parentCellElementList = getParentCellElementList();
  parentCellElementList.addEventListener('click', (e) => {
    let target = e.target;
    if (target.tagName !== 'LI') return;
    let index = target.dataset.index;
    handleCellClick(target, index);
  });
}
function resetGame() {
  // rs - current turn
